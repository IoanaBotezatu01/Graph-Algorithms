...
from graph import TripleDictGraph, write_graph_to_file, read_graph_from_file, write_modified_graph_to_file, \
    read_modified_graph_from_file
...
class UI:
    ...
    def minimum_cost_walk_ui(self):
        start_vertex = int(input("Enter start vertex = "))
        end_vertex = int(input("Enter end vertex = "))
        graph = self._graphs[self._current]
        if start_vertex < 0 or end_vertex < 0 or (start_vertex == end_vertex) or end_vertex >= graph.number_of_vertices :
            print("Vertices are not valid!")
        else:

            predecessors, dist = graph.find_minimum_cost_walk(start_vertex, end_vertex)

            if dist == float("inf"):
                print("There doesn't exist a path from the start vertex to the end vertex")
            else:
                #make the path starting from the end to the first vertex
                path = []
                backward = end_vertex
    ...
    def print_menu(self):
        print("Menu:\n"
              ...
              "26. Find the lowest cost walk between the given vertices.\n")
